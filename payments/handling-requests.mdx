...
Once a payment request is created - either for a [single payment](./single-payment.mdx) or to pay [multiple recipients at once](./bulk-payment.mdx) - it gets listed as a `Pending` request in the `Requests` section.
